    ...
    <div class="row">
        <div class="col-md-8 ">
            <div class="card" style="background-color: #ddc">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4">
                            <label for=""> <b>Receiving Mode</b> </label>
                            <select class="form-control" name="receiving_mode" id="receiving_mode"
                                class="selectpicker show-menu-arrow" data-style="btn-default btn-sm" data-width="fit"
                                tabindex="-98">
                        ...
                        <div class="col-md-4">
                            <br>
                            <a class="btn btn-secondary text-white" href="{{ url('receiving/manage') }}">Manage</a>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-4 text-end">
                            <a href="{{ url('items/new_item') }}" class="btn btn-info"> Add Items</a>
                        </div>
                    </div>
        ...
        <div class="col-md-4">
            <div class="card card-supplier">
                <div class="card-header text-center fw-bold"> Select Supplier (Optional)</div>
                <div class="card-body">
                    <div class="form-group" is="select-customar">
                        <input type="text" id="search_supplier" class="form-control"
                            placeholder="Start Typing Supplier's name.." tabindex="5">
                        <span class="text-danger" id="supplier_error"></span>
                        <div>Supplier: <span id="employee"></span></div>
                    </div>
                    <div class="form-group" is="select-employee">
                        ...
                        @endforeach
                           {{-- @if (Auth::user()->role == '5')
                           <option value="{{ Auth::user()->role }}">{{ Auth::user()->name }}
                               @else
                               @php
                               $employee = \App\Models\Employees::all();
                           @endphp
                           @foreach ($employee as $em)
                               <option selected value="{{ $em->id }}">{{ $em->first_name . ' ' . $em->last_name }}</option>
                           @endforeach
                           @endif --}}
                        </select>
                    </div>
                    ...
                    <div class="new-supplier text-center">
                        <a class="btn btn-info" href="{{ url('customer/create') }}"><i class="fas fa-user-tie"></i> New
                            Customer</a>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function() {
            ...
            $("#search_supplier").autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    $('span#employee').html('<span>' + ui.item.company_name +
                        '</span><input type="hidden" name="supplier_id" id="supplier_id" value="' +
            ...
            $('button#submit').click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response == 'Done') {
                            toastr.success('Receiving approved')
                        } else {
                            toastr.danger('Receiving Not approved')
                        }
                        setInterval(() => {
                            location.reload();
                        }, 800);
